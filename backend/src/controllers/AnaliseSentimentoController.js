...
      return res.status(400).json({ msg: 'Formato inválido. Envie um array de respostas.' });
    }

    // Chama o serviço de análise de sentimentos (Gemini)
    const resultado = await analyzeBatch(respostas, { language: idioma || 'pt' });
...
            attributes: [
                'id_resposta', 'resposta_texto', 'score', 'magnitude', 'label', 'theme', 'riskLevel', 'analysisSource'
            ],
    ...
    try {
        const insights = await Resposta.findAll({
            attributes: [
                ...
                [sequelize.fn('SUM', sequelize.literal("CASE WHEN label = 'negative' THEN 1 ELSE 0 END")), 'negativeCount']
            ],
            group: ['theme'],
